  ...
  texture.onload = () => {
    fetch("map.json")
      ...
      .then((data) => {
        ...
        turretMap = Array(map.length)
          .fill()
          .map((_) => Array(map[0].length).fill(0));
        path = data.path;
        ini = data.ini;
...
const transposeMap = () => {
  const tmpMap = Array(map[0].length)
    .fill()
    .map((_) => Array(map.length).fill(0));
  const tmpDetailMap = Array(map[0].length)
    .fill()
    .map((_) => Array(map.length).fill(0));
  const tmpTurretMap = Array(map[0].length)
    .fill()
    .map((_) => Array(map.length).fill(0));
...
const draw = function () {
  ...
  for (let i = 0; i < legitUsers.length; i++) {
    legitUsers[i].show();
  }
  ...
  if (selected) {
    c.strokeStyle = "blue";
    c.strokeRect(
      selected.pos.x * tileSize,
      selected.pos.y * tileSize,
      tileSize,
      tileSize
    );
  }
...
const update = function (time) {
  ...
  for (let i = legitUsers.length - 1; i >= 0; i--) {
    legitUsers[i].update();
    if (legitUsers[i].health <= 0) {
      money += legitUsers[i].value;
      fxs.push(
        new FX(legitUsers[i].pos, 1000, 1, 2, {
          grow: false,
          fade: true,
          rand: true,
        })
    ...
    } else if (legitUsers[i].win) {
      fxs.push(
        new FX(legitUsers[i].pos, 60, 1, 4, {
          grow: true,
          fade: true,
          rand: false,
        })
  ...
  for (let i = fxs.length - 1; i >= 0; i--) {
    fxs[i].update();
    if (fxs[i].isGone) fxs.splice(i, 1);
  }
...
const deployEnemies = () => {
  if (ie == 0) {
    ...
    tManager.addCallback(
      () => {
        enemies.push(
          new Enemy({ x: ini[1], y: ini[0] }, path, 15, enemyType, enemySpeed)
        );
      },
      performance.now(),
      1000,
...
const mouseClick = (e) => {
  ...
  if (
    turretType &&
    money - turretType.price >= 0 &&
    map[pos.y][pos.x] != 5 &&
    !detailMap[pos.y][pos.x] &&
    !turretMap[pos.y][pos.x]
  ) {
...
const feedback = (e) => {
  ...
  if (
    (spot < 5 || (spot > 5 && spot < 14) || spot > 13) &&
    !detailMap[pos.y][pos.x] &&
    !turretMap[pos.y][pos.x]
  )
    fc.strokeStyle = "black";
  else fc.strokeStyle = "red";
  fc.strokeRect(pos.x * tileSize, pos.y * tileSize, tileSize, tileSize);
  if (turretType) turretType.showPreview(pos);
};

const playPause = (e) => {
  e.preventDefault();
  if (u) u = cancelAnimationFrame(u);
  else update();
};
...
const reload = () => {
  ...
  if (yOffset == 0) xOffset = (xOffset + 5) % 15;
